...
def analyze_header(file: str) -> tuple:
    # Para garantizar esta funcionalidad se vuelve a leer el examen
    img = cv2.imread(f"./Ejercicio2/Tests/{file}",cv2.IMREAD_GRAYSCALE)   
    # Se define un umbral para obtener una imagen binaria
    th = img.max() * 0.9
    img_th = img < th
    # Se cuentan los píxeles blancos por fila y se determina un umbral para detectar líneas horizontales
    img_rows = np.sum(img_th,1)
    th_row = 400
    img_rows_th = img_rows > th_row
    # Se detecta la primer línea horizontal del examen que delimita al encabezado
    idx_1_lin_h = np.argwhere(img_rows_th)[0,0]
    # Se segmenta la imagen para captar al segmento que corresponde con el encabezado
    ...
    for i,nombre_campo in enumerate(nombes_campos_enc):
        ...
        connectivity = 8
        # Utilizamos componentes conectadas para encontrar a los caracteres del encabezado
    ...
    for file in os.listdir(dir_path):
        if file.endswith('.png'):  # Solo procesar archivos PNG
            ...
            if image is not None:
                ...
                id = file
                img_name,name ,date ,class_n = analyze_header(file)
